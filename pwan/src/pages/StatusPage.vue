...
export default {
  ...
  setup() {
    ...
    const urlLink = ref(path.STATUS_SEARCH
    );
    ...
    const fetchData = async () => {
      try {
        Loading.show();
        const response = await axios.get(path.STATUS_SEARCH,
          headers
        );
        if (response.data) {
    ...
    const createRecord = (record) => {
      try {
        const promise = axios.post(path.STATUS_CREATE,
          record,
          headers
        );
        promise
    ...
    const updateRecord = (record) => {
      try {
        ...
        const promise = axios.put(path.STATUS_UPDATE, 
          record,
          headers
        );
        promise
    ...
    const deleteItem = async () => {
      try {
        ...
        const response = await axios.post(path.STATUS_REMOVE,
          data,
          headers
        );
