...
export default {
  ...
  data() {
    ...
    const hint = computed(() => t("paymentstatus.hint"));
    const useStore = usePaymentStatusStore();
    const router = useRouter();
    const headers = SessionStorage.getItem("headers");
    const title = ref("Payment Status");
  ...
  methods: {
    fetchData() {
      try {
        ...
        axios
          .get(path.PAYMENTSTATUS_SEARCH, requestParams, this.headers)
          .then((response) => {
    ...
    setStoreCallRouter(record) {
      ...
      this.router.push("/paymentstatus/form");
    },
  },
  beforeCreate() {
