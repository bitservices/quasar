<template>
  <q-page padding>
    ...
    <div class='q-pa-md'>
      ...
      > 
        <template v-slot:top>
          <q-label>SMTP Configuration</q-label>
          <q-space />
...
export default {
  ...
  data() {
    const { t } = useI18n();
    const pageName = computed(()=> t('smtpconfig.pagename'))
    const hint = computed(()=> t('smtpconfig.hint'))
    ...
    saveRecord() { 
      if (this.$refs.integrationForm.validate()) {  
        try {
          ...
          if(this.action == 'add'){
                promise = axios.post(
            path.CLIENTSMTPCONFIG_CREATE,
            this.formData,
            this.headers
          ); 
          }else if(this.action == 'edit'){
                promise = axios.put(
            path.CLIENTSMTPCONFIG_UPDATE,
            this.formData,
            this.headers
              );
          } 
