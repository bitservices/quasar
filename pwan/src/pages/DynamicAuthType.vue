  ...
  <q-page padding>
    <div class='q-pa-md'>
      <q-table
        ...
        flat
        bordered
        title='Dynamic Authentication Types'
        :rows='rows'
        :columns='columns'
        row-key='name'
      ...
      >
        <template v-slot:top>
          <q-label>Dynamic Authentication Types</q-label>
          <q-space />
...
export default {
  ...
  setup() {
    ...
    const urlLink = ref(path.DYNAMICTYPE_SEARCH
    );
    ...
    const fetchData = async () => {
      try { 
        const response = await axios.get(path.DYNAMICTYPE_SEARCH,
          headers
        );
        if (response.data) {
    ...
    const createRecord = (record) => {
      try {
        const promise = axios.post(path.DYNAMICTYPE_CREATE,
          record,
          headers
        );
        promise
    ...
    const updateRecord = (record) => {
      try {
        ...
        const promise = axios.put(path.DYNAMICTYPE_UPDATE,
          record,
          headers
        );
        promise
    ...
    const deleteItem = async () => {
      try {
        ...
        const response = await axios.post(path.DYNAMICTYPE_REMOVE,
          data,
          headers
        );
