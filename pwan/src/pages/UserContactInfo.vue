<template>
  <q-page padding>
    <div class='q-pa-md'>
      ...
      >
        <template v-slot:top>
          <q-label>Contact Information</q-label>
          <q-space />
...
import { ref } from 'vue';
import UserContactInfoDialog from 'src/components/UserContactInfoDialog.vue';
  ...
  setup() {
    ...
    const columns = [
      ...
      },
      {
        ...
        field: (row) => row.whatsappNumber,
        sortable: true,
      },
    ];
    const urlLink = ref(path.CONTACTINFO_SEARCH);
    ...
    const createRecord = (record) => {
      try {
        const promise = axios.post(path.CONTACTINFO_CREATE, record, headers);
        promise
          .then((response) => {
