          ...
          <div v-if="toggleValue">
            <div class="row">
              ...
              >
                <q-badge
                  rounded
                  :label="`Unit(s):   ` + requiredUnit"
                  class="pwan-btton text-h6"
                />
              </div>
            </div>
            <q-input
...
export default {
    ...
    const columns = [
      {
        ...
        label: "Name",
        align: "left",
        field: (row) => row.donation.full_name,
        format: (val) => `${val}`,
        sortable: true,
      },
      {
  ...
  methods: {
    resolveRequiredUnit() {
      const msgLen =
        this.formData.message == null ? 0 : this.formData.message.length;
      if (msgLen > 160) {
    ...
    searchDonationData() {
      const requestParams = {
        params: {
          ...
          order_by: "donationItems__donationStatus__name",
        },
      };

      if (
        this.formData.donationStatus != null &&
        this.formData.donationStatus.value != null &&
        this.formData.donationStatus.value != ""
      ) {
        requestParams["params"]["status"] = this.formData.donationStatus.value;
      }

      if (
        this.formData.donationType != null &&
        this.formData.donationType.value != null &&
        this.formData.donationType.value != ""
      ) {
        requestParams["params"]["donationType"] =
          this.formData.donationType.value;
      }
      ...
      try {
        const requestParams = {
          params: {
            ...
            organisation: this.profile.organisation,
            order_by:
              "-donationItems__amount,donationItems__donationStatus__name",
          },
        };
    ...
    async downloadFilexxx() {
      ...
      if (
        ...
        this.formData.userId.value != null &&
    ...
    loadUserImage(userObj) {
      ...
      const promise = axios.get(path.USER_IMAGE, requestParam, this.headers);
      promise
        .then((response) => {
          this.imageFile =
            "data:image/jpeg;base64," + response.data.data.imageByte;
        })
        .catch((error) => {
          console.log(error);
        });
  ...
  mounted() {
    ...
    const promise = axios.get(path.DONATIONSTATUS_SEARCH, this.headers);
