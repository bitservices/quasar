    ...
    <div class="card-container" v-if="!toggleValue">
      ...
      >
        <router-link
          ...
          @click="handleClick('/userwebsitesetup')"
        >
          <q-card-section>
            <div class="text-h6">
              <q-icon name="fa fa-wifi" size="40px" />Website Setup
            </div>
      ...
      >
        <router-link
          ...
          class="q-card-link"
          style="text-decoration: none"
          @click="handleClick('/runcronejobs')"
        >
          <q-card-section>
            <div class="text-h6">
              <q-icon name="fa fa-job" size="40px" />Run Cron Jobs
            </div>
            <div>Run Cron Jobs</div>
          </q-card-section>
        </router-link>
      </q-card>
      <q-card v-if="isSupperUser" key="organisations-upload" class="my-card-d">
        <router-link
          to="/orgupload"
          class="q-card-link"
          style="text-decoration: none"
          @click="handleClick('/orgupload')"
        >
          <q-card-section>
            <div class="text-h6">
              <q-icon name="upload" size="40px" />Upload Organistions
            </div>
      ...
      <q-card v-for="item in items" :key="item.id" class="my-card-d text-white">
        <router-link
          :to="`${item.link}`"
          class="q-card-link"
          style="text-decoration: none"
        ...
        >
          <q-card-section>
            <div class="text-h6">
              <q-icon :name="item.icon" size="40px" />{{ item.title }}
            </div>
    ...
    <div class="q-pa-md row items-start q-gutter-md" v-else>
      ...
      >
        ...
        <q-card-section v-for="(field, index) in outstanding" :key="index">
          ...
          <hr />
          <div class="col-3">
            Current Debit: <b>N {{ field.currentDebit }}</b> | Paid:
            <b>N {{ field.paid }}</b>
          </div>
        </q-card-section>
      </q-card>
      ...
      <q-card
        dark
        bordered
        class="bg-red-9 my-card-d text-white"
      ...
      >
        ...
        <q-card-section v-for="(field, index) in contributions" :key="index">
          <div class="col-3">
            {{ field.organisation__name }}:
            <b>N {{ field.transactionAmount }}</b>
          </div>
        </q-card-section>
      </q-card>
      <q-card
        dark
        bordered
        class="bg-puple-9 my-card-d text-white"
      ...
      >
        ...
        <q-card-section v-for="(field, index) in attendance" :key="index">
          <div class="col-3">
            {{ field.organisation__name }}: <b>{{ field.count }}</b>
          </div>
        </q-card-section>
      </q-card>
      <q-card
        dark
        bordered
        class="bg-green-9 my-card-d text-white"
    ...
    return {
      ...
      items: [
        //{ id: 1, title: 'Real Estate Website', description: 'Create/Renew Your Real Estate Website',link:'/agencywebsite',gradient: 'linear-gradient(to right, #FF5733, #FFC300)',icon:'create'},
        {
          ...
          link: "/viewpayments",
          gradient: "linear-gradient(to right, #FF5733, #FFC300)",
          icon: "payment",
        },
        {
          id: 3,
        ...
        },
        {
          id: 4,
          title: "Turnel",
          description: "Turnelling to Different Client/Affilate",
          link: "/turnel",
          gradient: "linear-gradient(to right, #33FF57, #33FFC3)",
          icon: "loop",
        },
        {
          id: 5,
        ...
        },
        {
          ...
          link: "/selfattendance",
          gradient: "linear-gradient(to right, #3357FF, #C300FF)",
          icon: "check",
        },
        {
          id: 7,
          title: "Client/Center",
          description: "Client/Affilate/Center",
          link: "/client",
          gradient: "linear-gradient(to right, #33FF57, #33FFC3)",
          icon: "company",
        },
