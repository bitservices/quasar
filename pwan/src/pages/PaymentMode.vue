...
export default {
  ...
  data() {
    ...
    const hint = computed(() => t("paymentmode.hint"));
    const useStore = usePaymentModeStore();
    const router = useRouter();
    const headers = SessionStorage.getItem("headers");
    const title = ref("Payment Mode");
  ...
  methods: {
    fetchData() {
      try {
        ...
        axios
          .get(path.PAYMENTMODE_SEARCH, requestParams, this.headers)
          .then((response) => {
    ...
    setStoreCallRouter(record) {
      ...
      this.router.push("/paymentmode/form");
    },
  },
  beforeCreate() {
