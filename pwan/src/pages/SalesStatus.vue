...
export default {
  ...
  data() {
    ...
    const hint = computed(() => t("salestatus.hint"));
    const useStore = useSaleStatusStore();
    const router = useRouter();
    const headers = SessionStorage.getItem("headers");
    const title = ref("Sale Status");
  ...
  methods: {
    ...
    fetchData() {
      try {
        ...
        axios
          .get(path.SALESSTATUS_SEARCH, requestParams, this.headers)
          .then((response) => {
    ...
    setStoreCallRouter(record) {
      ...
      this.router.push("/salesstatus/form");
    },
  },
  beforeCreate() {
