      ...
      >
        <template v-slot:top>
          ...
          <StandingDataFormDialog
            ...
            label='Contact Configuration Type'
...
export default {
  ...
  setup() {
    ...
    const urlLink = ref(path.CONTACT_CONFIG_TYPE_SEARCH
    );
    ...
    const fetchData = async () => {
      try { 
        const response = await axios.get(path.CONTACT_CONFIG_TYPE_SEARCH,
          headers
        );
        if (response.data) {
    ...
    const createRecord = (record) => {
      try {
        console.log()
        const promise = axios.post(path.CONTACT_CONFIG_TYPE_CREATE,
          record,
          headers
        );
        promise
    ...
    const updateRecord = (record) => {
      try {
        ...
        const promise = axios.put(path.CONTACT_CONFIG_TYPE_UPDATE, 
          record,
          headers
        );
        promise
    ...
    const deleteItem = async () => {
      try {
        ...
        const response = await axios.post(path.DONATIONTYPE_REMOVE,
          data,
          headers
        );
