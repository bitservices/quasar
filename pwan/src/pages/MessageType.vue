...
export default {
  ...
  setup() {
    ...
    const urlLink = ref(path.MESSAGETYPE_SEARCH
    );
    ...
    const fetchData = async () => {
      try { 
        const response = await axios.get(path.MESSAGETYPE_SEARCH,
          headers
        );
        if (response.data) {
    ...
    const createRecord = (record) => {
      try {
        const promise = axios.post(path.MESSAGETYPE_CREATE,
          record,
          headers
        );
        promise
    ...
    const updateRecord = (record) => {
      try {
        ...
        const promise = axios.put(path.MESSAGETYPE_UPDATE,
          record,
          headers
        );
        promise
    ...
    const deleteItem = async () => {
      try {
        ...
        const response = await axios.post(path.MESSAGETYPE_REMOVE,
          data,
          headers
        );
