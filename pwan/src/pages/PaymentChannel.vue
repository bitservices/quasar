<template>
  <q-page padding>
    <div class='q-pa-md'>
      ...
      >
        <template v-slot:top>
          <q-label>Payment Channels</q-label>
          <q-space />
...
export default {
  ...
  setup() {
    ...
    const urlLink = ref(path.PAYMENTCHANNEL_SEARCH
    );
    ...
    const fetchData = async () => {
      try { 
        const response = await axios.get(path.PAYMENTCHANNEL_SEARCH,
          headers
        );
        if (response.data) {
    ...
    const createRecord = (record) => {
      try {
        const promise = axios.post(path.PAYMENTCHANNEL_CREATE,
          record,
          headers
        );
        promise
    ...
    const updateRecord = (record) => {
      try {
        ...
        const promise = axios.put(path.PAYMENTCHANNEL_UPDATE,
          record,
          headers
        );
        promise
    ...
    const deleteItem = async () => {
      try {
        ...
        const response = await axios.post(path.PAYMENTCHANNEL_REMOVE,
          data,
          headers
        );
