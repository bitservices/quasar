...
export default {
  ...
  data() {
     const { t } = useI18n() 
    const pageName = computed(()=> t('usermsgintegration.pagename'))
    const hint = computed(()=> t('usermsgintegration.hint'))
  ...
  methods: {
    ...
    searchData() {
      ...
      try {
        ...
        const promise = axios.get(
          path.USER_MESSAGINGINTEGRATION_SEARCH,
          requestParams,
          this.headers
        ); 
        promise
    ...
    completePaymentTransaction(data, response){
      try{
              ...
              const promise = axios.post(path.USER_MESSAGINGINTEGRATION_CREATE, paymentItems, this.headers);
            promise
              .then((response) => {
                // Extract data from the response
                const result = response.data;  
