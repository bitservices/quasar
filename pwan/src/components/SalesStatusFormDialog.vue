...
import { useSaleStatusStore } from "src/stores/saleStatusStore";
...
export default {
  ...
  data() {
    ...
    const { t } = useI18n();
    const pageName = computed(() => t("salestatus.pagename"));
    const hint = computed(() => t("salestatus.maintain.hint"));
    const controlWidth = viewportWidth * 0.9; // 90% of the viewport width
    const controlHeight = viewportHeight * 0.9; // 90% of the viewport height
    const dialogWidth = controlWidth + "px";
    const dialogHeight = controlHeight + "px";
    const headers = SessionStorage.getItem("headers");
    const useStore = useSaleStatusStore();
    ...
    executeRecord() {
      if (this.$refs.formRef.validate()) {
        ...
        if (this.useStore.mode == "create") {
          promise = axios.post(path.SALESSTATUS_CREATE, record, this.headers);
        }
        if (this.useStore.mode == "update") {
          promise = axios.put(path.SALESSTATUS_UPDATE, record, this.headers);
        } else if (this.useStore.mode == "delete") {
          console.log("calling deactivate >>>>>>>>>>>>>");
          promise = axios.post(path.SALESSTATUS_REMOVE, record, this.headers);
        }
