...
import { usePaymentModeStore } from "src/stores/paymentModeStore";
...
export default {
  ...
  data() {
    ...
    const { t } = useI18n();
    const pageName = computed(() => t("paymentmode.pagename"));
    const hint = computed(() => t("paymentmode.maintain.hint"));
    const controlWidth = viewportWidth * 0.9; // 90% of the viewport width
    const controlHeight = viewportHeight * 0.9; // 90% of the viewport height
    const dialogWidth = controlWidth + "px";
    const dialogHeight = controlHeight + "px";
    const headers = SessionStorage.getItem("headers");
    const useStore = usePaymentModeStore();
    ...
    executeRecord() {
      if (this.$refs.formRef.validate()) {
        ...
        if (this.useStore.mode == "create") {
          promise = axios.post(path.PAYMENTMODE_CREATE, record, this.headers);
        }
        if (this.useStore.mode == "update") {
          promise = axios.put(path.PAYMENTMODE_UPDATE, record, this.headers);
        } else if (this.useStore.mode == "delete") {
          console.log("calling deactivate >>>>>>>>>>>>>");
          promise = axios.post(path.PAYMENTMODE_REMOVE, record, this.headers);
        }
