...
export default {
  ...
  data() {
    ...
    const useStore = usePaymentStatusStore();
    ...
    executeRecord() {
      if (this.$refs.formRef.validate()) {
        ...
        if (this.useStore.mode == "create") {
          promise = axios.post(path.PAYMENTSTATUS_CREATE, record, this.headers);
        }
        if (this.useStore.mode == "update") {
          promise = axios.put(path.PAYMENTSTATUS_UPDATE, record, this.headers);
        } else if (this.useStore.mode == "delete") {
          console.log("calling deactivate >>>>>>>>>>>>>");
          promise = axios.post(path.PAYMENTSTATUS_REMOVE, record, this.headers);
        }
        ...
        promise
          .then((response) => {
            ...
            this.router.push("/paymentstatus");
          })
          .catch((error) => {
